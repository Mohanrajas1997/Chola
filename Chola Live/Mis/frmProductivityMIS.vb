...
#End Region
    ...
    Private Sub GridProperty()
        ...
        With msfGrid
            ...
            .set_ColWidth(lnCol, 1440 * 0.75)
            .set_ColAlignment(lnCol, 7)
            .set_TextMatrix(lnRow, lnCol, "FinnOne Entry")
            .set_TextMatrix(lnRow + 1, lnCol, "Count")
    ...
    Private Sub LoadData()
        ...
        With msfGrid
            ...
            For i = .FixedRows To .Rows - 1
                ...
                lsSql = ""
                lsSql &= " select "
                lsSql &= " count(p.packet_gid) as pkt_count"
                lsSql &= " from chola_trn_tpacket as p "
                lsSql &= " where p.gnsa_lock_date >= '" & lsFrom & "' "
                lsSql &= " and p.gnsa_lock_date <= '" & lsTo & "' "
                lsSql &= " and p.gnsa_lock_by = '" & lsUserName & "' "
                ...
                ' FinnOne packet Count
                lsSql = ""
                lsSql &= " select "
                lsSql &= " count(p.packet_gid) as pkt_count"
                lsSql &= " from chola_trn_tpacket as p "
                lsSql &= " where p.fin_lock_date >= '" & lsFrom & "' "
                lsSql &= " and p.fin_lock_date <= '" & lsTo & "' "
                lsSql &= " and p.fin_lock_by = '" & lsUserName & "' "

                Call gpDataSet(lsSql, "pkt", gOdbcConn, ds)

                If ds.Tables("pkt").Rows.Count > 0 Then
                    .set_TextMatrix(lnRow, lnCol + 4, ds.Tables("pkt").Rows(0).Item("pkt_count").ToString)
                End If
                ...
                If ds.Tables("pkt").Rows.Count > 0 Then
                    .set_TextMatrix(lnRow, lnCol + 5, ds.Tables("pkt").Rows(0).Item("chq_count").ToString)
                End If
                ...
                If ds.Tables("pkt").Rows.Count > 0 Then
                    .set_TextMatrix(lnRow, lnCol + 6, ds.Tables("pkt").Rows(0).Item("chq_count").ToString)
                End If
