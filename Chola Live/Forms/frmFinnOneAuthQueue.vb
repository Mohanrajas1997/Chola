Public Class frmFinnOneAuthQueue
    ...
    Private Sub LoadData()
        ...
        If rdoShowall.Checked = True Then
            ...
            lsSql &= " from chola_trn_tpacket "
            lsSql &= " inner join chola_mst_tagreement on agreement_gid=packet_agreement_gid "
            lsSql &= " where packet_status & " & GCPACKETFINONEAUDITED & " = " & GCPACKETFINONEAUDITED & " "
            lsSql &= " and packet_status & " & GCPACKETFINONEAUTH & "= 0"
            lsSql &= " and packet_status &" & GCPACKETFINONEAUTHREJECT & "= 0"
        ...
        With dgvsummary
            ...
            lobjbtnColumn.HeaderText = "Complete"
            lobjbtnColumn.Name = "Auth Complete"
            lobjbtnColumn.Text = "Auth Complete"

            .Columns.Add(lobjbtnColumn)

            lobjbtnColumn = New DataGridViewButtonColumn
            lobjbtnColumn.HeaderText = "Rejection"
            lobjbtnColumn.Name = "Reject"
            lobjbtnColumn.Text = "Reject"

            .Columns.Add(lobjbtnColumn)


            For i = 0 To .RowCount - 1
                .Rows(i).Cells(n).Value = "Auth Complete"
                .Rows(i).Cells(n + 1).Value = "Auth Reject"
            Next i
    ...
    Private Sub dgvsummary_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs)
    ...
    Private Sub dgvsummary_CellContentClick(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvsummary.CellContentClick
        Dim lnPacketId As Long
        Dim lsSql As String
       
        If e.RowIndex >= 0 Then
            Select Case e.ColumnIndex
                Case dgvsummary.Columns.Count - 2
                    If MessageBox.Show("Are you sure you want to submit ?", gProjectName, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then


                        lnPacketId = dgvsummary.Rows(e.RowIndex).Cells("packet_gid").Value

                        lsSql = ""
                        lsSql &= " Update chola_trn_tpacket set"
                        lsSql &= " packet_status=packet_status |" & GCPACKETFINONEAUTH & ""
                        lsSql &= " where packet_gid=" & lnPacketId
                        gfExecuteQry(lsSql, gOdbcConn)

                        Call LoadData()
                    End If
                Case dgvsummary.Columns.Count - 1
                    If MessageBox.Show("Are you sure you want to Reject ?", gProjectName, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                        lnPacketId = dgvsummary.Rows(e.RowIndex).Cells("packet_gid").Value

                        lsSql = ""
                        lsSql &= " Update chola_trn_tpacket set"
                        lsSql &= " packet_status=packet_status |" & GCPACKETFINONEAUTHREJECT & ""
                        lsSql &= " where packet_gid=" & lnPacketId
                        gfExecuteQry(lsSql, gOdbcConn)
        ...
        With dgvsummary
            If e.RowIndex >= 0 Then
                lnPacketId = .Rows(e.RowIndex).Cells("packet_gid").Value
            End If
        End With
