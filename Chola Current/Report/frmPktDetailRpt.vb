Public Class frmPktDetailRpt
    ...
    Private Sub frmPacketReport_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        With cboStatus
            ...
            .Items.Add("FinnOne Matched")
            .Items.Add("FinnOne UnMatched")
        End With
    ...
    Private Sub PdcData()
        ...
        Try
            ...
            Select Case cboStatus.Text
                ...
                Case "FinnOne Matched"
                    lsCond &= " and c.chq_pdc_gid >0"
                Case "FinnOne UnMatched"
                    lsCond &= " and c.chq_pdc_gid=0"
            End Select

            If Val(txtPktId.Text) > 0 Then lsCond &= " and p.packet_gid = " & Val(txtPktId.Text) & " "

            If txtMicrCode.Text.Contains("%") Or txtMicrCode.Text.Contains("_") Then
                If txtMicrCode.Text <> "" Then lsCond &= " and c.chq_micrcode like '" & QuoteFilter(txtMicrCode.Text) & "%' "
            Else
            ...
            lsFld &= " case when c.chq_finnone_desc=1 then 'Chq Date Mismatch' when c.chq_finnone_desc=2 then 'Chq No Mismatch' when c.chq_finnone_desc=4 then 'Amount Mismatch' when c.chq_finnone_desc=16 then 'Acc No Mismatch' else '' end as 'FinnOne Chq Desc',"
            ...
            lsSql &= " left join chola_trn_tscan as s on s.scan_packet_gid=p.packet_gid and s.scan_chq_gid=c.entry_gid"
    ...
    Private Sub SpdcData()
        ...
        Try
            ...
            Select Case cboStatus.Text
                ...
                Case "FinnOne Matched"
                    lsCond &= " and c.chqentry_pdc_gid >0"
                Case "FinnOne UnMatched"
                    lsCond &= " and c.chqentry_pdc_gid=0"
            End Select
            ...
            If txtMicrCode.Text.Contains("%") Or txtMicrCode.Text.Contains("_") Then
                If txtMicrCode.Text <> "" Then lsCond &= " and c.chqentry_micrcode like '" & QuoteFilter(txtMicrCode.Text) & "%' "
            Else
            ...
            lsFld &= " b.box_no as 'Box No',p.packet_ismultiplebank as 'Multiple Bank',"
            lsFld &= " case when c.chqentry_finnone_desc=1 then 'Chq Date Mismatch' when c.chqentry_finnone_desc=2 then 'Chq No Mismatch' when c.chqentry_finnone_desc=4 then 'Amount Mismatch' when c.chqentry_finnone_desc=16 then 'Acc No Mismatch' else '' end as 'FinnOne Chq Disc',"
            lsFld &= " p.packet_gid as 'Packet Id',p.packet_inward_gid as 'Inward Id',a.agreement_gid as 'Agreement Id',"
            ...
            lsSql &= " left join chola_trn_tscan as s on s.scan_packet_gid=p.packet_gid and s.scan_chq_gid=c.chqentry_gid"
            lsSql &= " left join chola_trn_tbox b on p.packet_box_gid = b.box_gid "
            lsSql &= " left join chola_mst_tagreement a on p.packet_agreement_gid = a.agreement_gid "
            lsSql &= " where true "
            lsSql &= lsCond
    ...
    Private Sub EcsData()
        ...
        Try
            ...
            If txtMicrCode.Text.Contains("%") Or txtMicrCode.Text.Contains("_") Then
                If txtMicrCode.Text <> "" Then lsCond &= " and c.ecsemientry_micrcode like '" & QuoteFilter(txtMicrCode.Text) & "%' "
            Else
