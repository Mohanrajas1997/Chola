    ...
    Private Sub ImportExcel(ByVal FilePath As String, ByVal SheetName As String)
        ...
        Try
            ...
            For liIndex As Integer = 0 To lExcelDatatable.Rows.Count - 1
                ...
                'Check for Duplicate
                lssql = " Select pdc_gid from chola_trn_tpdcfile where " & _
                            " pdc_parentcontractno='" & lExcelDatatable.Rows(liIndex).Item("PARENT_CONTRACT_NUMBER").ToString.Trim & "' and pdc_chqamount =" & Val(QuoteFilter(lExcelDatatable.Rows(liIndex)("CHEQUEAMOUNT").ToString)) & " " & _
                            " and pdc_contractno='" & lExcelDatatable.Rows(liIndex).Item("CONTRACT_NUMBER").ToString.Trim & "' and pdc_chqdate='" & Format(CDate(lExcelDatatable.Rows(liIndex).Item("CHEQUEDATE").ToString), "yyyy-MM-dd") & "' " & _
                            " and pdc_chqno='" & lExcelDatatable.Rows(liIndex).Item("CHEQUESNO").ToString.Trim & "' and pdc_acc_no=" & lExcelDatatable.Rows(liIndex).Item("BANK_ACCOUNTNO").ToString.Trim & "' and chq_rec_slno=1 "
                ...
                If Val(lsFileGid) > 0 Then
                    lidup += 1
                    liIsDuplicate += 1
                    lsDuplicatedtl = Now() & " Duplicate exist " & lExcelDatatable.Rows(liIndex).Item("PARENT_CONTRACT_NUMBER").ToString.Trim & " " & lExcelDatatable.Rows(liIndex).Item("CONTRACT_NUMBER").ToString.Trim & " " & lExcelDatatable.Rows(liIndex).Item("CHEQUESNO").ToString.Trim & " " & lExcelDatatable.Rows(liIndex).Item("BANK_ACCOUNTNO").ToString.Trim
                    PrintLine(1, lsDuplicatedtl)
                    PrintLine(1, lsDuplicatedtl)
                    GoTo GoNext1
                End If
                ...
                lssql &= " and pdc_chqdate='" & Format(CDate(lExcelDatatable.Rows(liIndex).Item("CHEQUEDATE").ToString), "yyyy-MM-dd") & "'"
                lssql &= " and pdc_acc_no=" & lExcelDatatable.Rows(liIndex).Item("BANK_ACCOUNTNO").ToString.Trim & "'"
                lssql &= " and pdc_chqamount =" & Val(QuoteFilter(lExcelDatatable.Rows(liIndex)("CHEQUEAMOUNT").ToString)) & ""
