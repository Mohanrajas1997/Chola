Public Class frmLooseChqEntryForBatch
    Dim msSql As String
    ...
    Private Sub btnFind_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFind.Click
        ...
        Dim lnSlno As String

        lnSlno = Val(txtSNo.Text.Trim)
        If lnSlno = "" Then
            MsgBox("Please Input the Serial no", MsgBoxStyle.Information, gProjectName)
        Else
            Try
                SearchDialog = New Search(gOdbcConn, _
                                 ...
                                 " 1 = 1 and a.loosechqentry_deleteflag = 'N' and a.loosechqentry_sno = " & lnSlno)
        ...
        Try
            ...
            Else
                If MsgBox("Are you sure want to delete this record?", MsgBoxStyle.YesNo + MsgBoxStyle.DefaultButton2, gProjectName) = MsgBoxResult.Yes Then
                    ...
                    msSql &= " loosechqentry_deleteflag='Y',loosechqentry_deletedate = sysdate(),"
                    msSql &= " loosechqentry_batch_gid=0,"
                    msSql &= " loosechqentry_deleteby = '" & QuoteFilter(gUserName) & "' "
                    ...
                    ' Get chq Amount
                    Dim lnchqamt As String

                    msSql = ""
                    msSql &= " select chq_amount from chola_trn_tpdcentry "
                    msSql &= " where entry_gid=" & Val(txtChqEntryId.Text)

                    lnchqamt = Val(gfExecuteScalar(msSql, gOdbcConn))
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            If cboAgrNo.SelectedIndex = -1 Or cboAgrNo.Text.Trim = "" Then
                MsgBox("Please select the Agreement No !", MsgBoxStyle.Information, gProjectName)
                cboAgrNo.Focus()
                Exit Sub
            End If
            ...
            msSql = ""
            msSql = " select chq_batch_gid "
            msSql &= " from chola_trn_tpdcentry "
            msSql &= " where chq_no='" & txtChqNo.Text.Trim & "'"
            msSql &= " and chq_amount=" & Val(txtChqAmount.Text.Trim)
            msSql &= " and chq_date='" & Format(CDate(dtpChqDate.Text), "yyyy-MM-dd") & "'"
            msSql &= " and chq_agreement_gid=" & Val(cboAgrNo.SelectedValue.ToString)
            'Mohan
            txtBatch_gid.Text = gfExecuteScalar(msSql, gOdbcConn)
            ...
            If lnLooseChqId = 0 Then
                ...
                msSql &= " inner join chola_trn_tbatch as b on c.chq_batch_gid = b.batch_gid and b.batch_istally = 'N' "
                ...
                msSql &= " or c.chq_status & " & GCPRESENTATIONPULLOUT & " > 0 ) "
                ...
                lnBatchId = Val(txtBatch_gid.Text)
                'insert in loosechqentry table
                msSql = ""
                msSql &= " insert into chola_trn_tloosechqentry (loosechqentry_sno,loosechqentry_pdcgid,loosechqentry_batch_gid,loosechqentry_remarks,"
                ...
                ' delete from chqentry table
                msSql = ""
                msSql &= " delete from chola_trn_chqentry "
                msSql &= " where chqentry_entry_gid=" & lnChqEntryId
                ...
                msSql = ""
                msSql &= " update chola_trn_tpdcentry "
                msSql &= " set chq_status=(chq_status | (" & GCLOOSECHQ & " | " & GCPRESENTATIONDE & " | " & GCPRESENTATIONPULLOUT & ")) "
                msSql &= " ^ (" & GCPRESENTATIONDE & " | " & GCPRESENTATIONPULLOUT & ") "
                ...
                msSql = ""
                msSql &= " update chola_trn_tbatch "
                msSql &= " set batch_totalchq = batch_totalchq - 1, "
                msSql &= " batch_entrychq = batch_entrychq - 1 ,batch_totalchqamt = batch_totalchqamt - " & lnChqAmt
                msSql &= " ,batch_entrychqamt = batch_entrychqamt - " & lnChqAmt
                msSql &= " where batch_gid=" & lnBatchId
            ...
            If MsgBox("Do you want to add another record ?", MsgBoxStyle.YesNo, gProjectName) = MsgBoxResult.Yes Then
                Dim lsBatchGid As Long
                msSql = ""
                msSql &= " select max(loosechqentry_sno) from chola_trn_tloosechqentry "
                msSql &= " where loosechqentry_deleteflag = 'N' "

                lsBatchGid = gfExecuteScalar(msSql, gOdbcConn)

                txtSNo.Text = lsBatchGid + 1
                txtChqNo.Focus()
            End If
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Information, gProjectName)
    ...
    Private Sub cboAgrNo_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboAgrNo.GotFocus
        ...
        If Val(txtChqAmount.Text) > 0 Then
            If txtChqNo.Text.Trim = "" Then
                ...
                Exit Sub
            ElseIf Not IsDate(dtpChqDate.Text) Then
                MsgBox("please enter Valid Cheque Date", MsgBoxStyle.Information)
                Exit Sub
            Else
                msSql = " select distinct agreement_gid,concat(shortagreement_no,'-',ifnull(chq_accno,'')) as shortagreement_no "
                msSql &= " from chola_trn_tpdcentry "
                msSql &= " inner join chola_mst_tagreement on agreement_gid=chq_agreement_gid"
                msSql &= " where chq_no='" & txtChqNo.Text.Trim & "'"
                msSql &= " and chq_amount=" & Val(txtChqAmount.Text.Trim)
                msSql &= " and chq_date='" & Format(CDate(dtpChqDate.Text), "yyyy-MM-dd") & "'"

                gpBindCombo(msSql, "shortagreement_no", "agreement_gid", cboAgrNo, gOdbcConn)

                If cboAgrNo.Items.Count = 0 Then Exit Sub
        ...
        'Call gpBindCombo(msSql, "shortagreement_no", "agreement_gid", cboAgrNo, gOdbcConn)
    ...
    Private Sub frmLooseChqEntryForBatch_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        GenerateSlno()
    End Sub

    Private Sub GenerateSlno()
        Dim lsBatchGid As Long
        txtRemark.Text = "Remove From Batch."
        msSql = ""
        msSql &= " select max(loosechqentry_sno) from chola_trn_tloosechqentry "
        msSql &= " where loosechqentry_deleteflag = 'N' "
    ...
    Private Sub txtChqAmount_LostFocus(sender As Object, e As EventArgs) Handles txtChqAmount.LostFocus
        If Val(txtChqAmount.Text) > 0 Then
            If txtChqNo.Text.Trim = "" Then
                MsgBox("please enter Cheque No", MsgBoxStyle.Information)
                Exit Sub
