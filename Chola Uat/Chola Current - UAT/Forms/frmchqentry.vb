...
Public Class frmchqentry
    ...
    Private Sub txtchequeamt_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtchequeamt.LostFocus
        If Val(txtchequeamt.Text) > 0 Then
            ...
            ElseIf Not IsDate(mtxtchqdate.Text) Then
                ...
                Exit Sub
            Else
                lssql = " select distinct agreement_gid,concat(shortagreement_no,'-',ifnull(chq_accno,'')) as shortagreement_no "
    ...
    Private Sub FillEntryGrid()
        ...
        lssql = " select @slno:= @slno + 1 as 'SL No',packet_gnsarefno as 'GNSA REF#',shortagreement_no as 'Short Agreement No',chq_no as 'Cheque No',date_format(chq_date,'%d-%m-%Y') as 'Cheque Date',chq_amount as 'Amount',"
        ...
        With ds.Tables("pdc")
            Select Case .Rows.Count
                Case 1
                    ...
                    'lssql &= " where chqentry_isactive = 'Y' and chqentry_entry_gid = " & lnPdcId & " "
                    lssql &= " wherechqentry_entry_gid = " & lnPdcId & " "
                    ...
                    'changes done by Mohan on 03-04-2024
                    ' loose cheque
                    If (lnStatus And GCLOOSECHQ) = GCLOOSECHQ Then
                        'delete from loosechqentry
                        lssql = ""
                        lssql &= " update chola_trn_tloosechqentry set "
                        lssql &= " loosechqentry_deleteflag='Y',loosechqentry_deletedate = sysdate(),"
                        'lssql &= " loosechqentry_batch_gid=0,"
                        lssql &= " loosechqentry_deleteby = '" & QuoteFilter(gUserName) & "' "
                        lssql &= " where loosechqentry_pdcgid = '" & lnPdcId & "' "
                        ...
                        lssql &= " set chq_status = chq_status ^ " & GCLOOSECHQ & " "
                        lssql &= " where entry_gid=" & lnPdcId
                        lssql &= " and chq_status & " & GCLOOSECHQ & " > 0 "
