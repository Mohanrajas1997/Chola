Public Class frmFinnoneImageQueue
    ...
    Private Sub btnrefresh_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnrefresh.Click
        ...
        If txtgnsarefno.Text <> "" Then
            ...
            lssql = ""
            lssql &= " select fin_lock_flag,fin_lock_by "
            lssql &= " from chola_trn_tpacket "
            lssql &= " inner join chola_mst_tagreement on agreement_gid=packet_agreement_gid "
            lssql &= " where shortagreement_no='" & dgvsummary.Rows(0).Cells("Short Agreement No").Value.ToString & "'"
            lssql &= " and packet_gnsarefno= '" & dgvsummary.Rows(0).Cells("GNSA REF#").Value.ToString & "'"
            ...
            If ds.Tables("Packet").Rows.Count = 1 Then
                lsLockFlag = ds.Tables("Packet").Rows(0).Item("fin_lock_flag").ToString
                lsLockBy = ds.Tables("Packet").Rows(0).Item("fin_lock_by").ToString
            End If
            ...
            Dim frmImageEntry As New frmFinnOneImageEntry(dgvsummary.Rows(0).Cells("GNSA REF#").Value.ToString, dgvsummary.Rows(0).Cells("Short Agreement No").Value.ToString)
    ...
    Private Sub LoadData()
        ...
        If rdoShowall.Checked = True Then
            ...
            lsSql &= " from chola_trn_tpacket "
            lsSql &= " inner join chola_mst_tagreement on agreement_gid=packet_agreement_gid "
            lsSql &= " where packet_status & " & GCPACKETSCANCOMPLETED & " > 0 "
            lsSql &= " and packet_status & " & GCPACKETFINONEAUDITED & " = 0 "
    ...
    Private Sub dgvsummary_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvsummary.CellContentDoubleClick
        ...
        Dim frmImageEntry As New frmFinnOneImageEntry(dgvsummary.Rows(e.RowIndex).Cells("GNSA REF#").Value.ToString, dgvsummary.Rows(e.RowIndex).Cells("Short Agreement No").Value.ToString)
        frmImageEntry.ShowDialog()
        'txtgnsarefno.Text = ""
