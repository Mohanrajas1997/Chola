        ...
        Try
            ...
            For liIndex As Integer = 0 To lExcelDatatable.Rows.Count - 1
                ...
                If rdoPkt.Checked Then
                    ...
                    lssql &= " case when packet_status & " & GCIPACKETPULLOUT & " = 0 then '' else 'Packet Pulledout' end as Remarks, "
                    lssql &= " almaraentry_cupboardno as 'Cupboard No',almaraentry_shelfno as 'Shelf No',almaraentry_boxno as 'Box No',packet_status, "
                    lssql &= " packetpullout_reason,packet_remarks "
                    'lssql &= " make_set(packet_status," & lsTxt & ") as 'Packet' "
                    ...
                    lssql &= " left join chola_trn_tpacketpullout on packet_gid = packetpullout_packet_gid"
                ...
                If dragreement.HasRows Then
                    While dragreement.Read
                        ...
                        If rdoPkt.Checked = True Then
                            If (dragreement.Item("packet_status") And (GCIPACKETPULLOUT Or GCREJECTENTRY)) > 0 Then
                                lbAddFlag = False
                                liinvalid += 1

                                lsstr = ""
                                lsstr &= Now() & "  Packet Already Pulled Out : " & QuoteFilter(lExcelDatatable.Rows(liIndex)("AGREEMENTNO").ToString) & ","
                                PrintLine(1, lsstr)
                            End If
                        End If
                        ...
                        If lbAddFlag = True Then
                            ...
                            objdt.Rows(objdt.Rows.Count - 1).Item("Remark") = dragreement.Item("Remarks").ToString
                        End If
                    End While
                Else
                    liinvalid += 1
